...
AI人格に実プレイ風ログを生成させる試走は `bash scripts/run_ai_persona_playtest.sh --turns 8` で実行できます。既定では `personas/kenji_style_player.yaml` を読みます。これは実セッションの保存ファイルをAIに直接編集させず、Codex CLIにread-onlyで生ログを作らせてから分析する安全寄りの導線です。
描写品質を支える Style Layer については `ARCHITECTURE.md` と `REQUIREMENTS.md` を参照してください。
漫画/作品から抽象化した物語構造ストックは `prompt/story_reference.md` にあり、各 session では `design/story_reference.md` / `design/story_spine.md` / `design/organization_cast.md` に変換して使います。
