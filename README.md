...
AIプレイ検証系は、回帰確認用の `PI Player`、実プレイ風ログ生成用の `AI Persona Playtest`、将来構想の `AI Player Harness` を分けて扱います。使い分けは下の「AIプレイ検証」章を参照してください。
...
| AI Player Harness | 将来構想。まだ本格実装しない。複数personaで20〜100ターン程度の実プレイ風検証を回し、personaごとの差や長期破綻を見る上位ハーネスとして設計する。 | GM破綻、ヒロイン自律性、恋愛/生活/事件の絡み、Knowledge Boundary違反、漫画化候補、save/resume/pre_compress/check_session_integrity連携 |
...
- raw log 生成後の保存反映、session更新、pre_compress、integrity check は別処理です。
