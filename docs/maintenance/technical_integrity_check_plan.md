...
| 10. 削除系コマンドの安全性 | cleanup が誤削除や tracked file 削除をしないことを見る | `play.sh cleanup-sessions` は tracked / unsafe name を判定。repo check は未検査 | dry-run で候補と削除不可が分かる | dry-run smoke 未実施 | tracked file や unsafe path を削除候補にする | `cleanup-sessions --dry-run` を `--full` で smoke |
