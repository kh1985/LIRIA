...
現状は root 直下の設計文書と保守メモの整理が進んでいる。macOS launcher 群は不要判断により削除済みで、起動導線は `./liria` / `bash play.sh` に一本化する。
...
- 第4弾で移動済み: `docs/maintenance/core_notes.md`（旧 `CORE.md`）
- 第5弾で削除済み: macOS launcher 群

`docs/maintenance/core_notes.md` は保守メモであり、実行時 prompt assembly の正本ではない。
...
```text
root markdown / launcher files: 9
docs: 13
personas: 4
prompt: 17
scripts: 19
...
| `docs/maintenance/core_notes.md` | 移動済み | 旧 `CORE.md`。現行promptの保守要約で、実行正本ではない | ARCHITECTURE.md, TODO.md を更新済み |
...
| 対象 | 理由 | 方針 |
|---|---|---|
| macOS launcher 群 | 今後は `./liria` からのみ起動するため不要 | 削除済み。代替 launcher は作らない |
...
| `docs/maintenance/core_notes.md` | `ARCHITECTURE.md`, `TODO.md` を更新済み |
...
macOS launcher は不要判断により削除済み。今後の起動導線は `./liria` / `bash play.sh` を正とする。

## 次回以降の安全手順

1. 必要なら `docs/architecture`, `docs/validation`, `docs/maintenance`, `docs/planning`, `docs/product` を作る。
2. 第1弾から第4弾までの移動済みファイルは、現行pathを前提として扱う。
