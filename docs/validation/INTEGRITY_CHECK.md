...
- `relationships.md` の関係フックと、各ヒロインの現在地が今のフェーズに合っている
- `relationships.md` に `Heroine Crisis Role` があり、frontline / support / civilian / wildcard と危機時の行動傾向が読める
- `relationships.md` のヒロイン知識スコープが `gm.md` と矛盾していない
...
- `bond` `AFFINITY` が `current/relationships.md` と矛盾していない
...
- `GM-only Body Profile` は成人ヒロイン/成人主要NPCの cast file にだけあり、正確な身長や3サイズ等が本文、hotset、公開用manga promptへ漏れていない
...
- `manga_pipeline.md` に export type、package 構成、image gen tasks 分解、相談型/実生成明示型の違い、実生成前の確認方針が残っている
...
- `source.md`、`brief.md`、`character_refs.md`、`name.md`、`panel_prompts.md`、`lettering.md`、`image_gen_tasks.md`、`job_status.md`、`publish_notes.md` が揃っている
- 相談型では `image_gen_tasks.md` はタスク分解であり、実生成済み扱いにしていない
- 実生成明示型では `job_status.md` に `queued` / `generating` / `generated` / `failed` と、生成参照や失敗理由が残っている
- manga export の default visual style は漫画/イラストであり、実写/フォトリアルはユーザー明示時だけになっている
...
- manga export package から `image_gen_tasks.md` または `job_status.md` が消えている、または相談型でユーザー確認前に実生成済み扱いになっている
