...
`current/case.md` は `Heroine-Driven Incident Gate` を通った `design/story_spine.md` の Concrete Story Arc から切り出した現在の見える slice であり、単独の物語生成正本ではない。
handles や objects だけで始めず、「誰が心配していて、何を失いたくないか」を先に可視化する。
...
5. Concrete Story Arc の前に `Heroine-Driven Incident Gate` を通す。
   - `heroine daily life`, `daily life at risk`, `why normal windows fail`, `why protagonist can enter`, `shared action`, `kyun point` を各1行で確認する。
   - 主人公だけで解けるなら、事件ではなく恋愛RPGイベントとして組み直す。
6. `design/story_spine.md` に Concrete Story Arc を作る。
   - `core truth`, `why now`, `surface case`, `main antagonistic pressure`, `key objects / places / records`, `clue chain`, `NPC intent`, `midpoint reveal`, `end choice`, `if ignored`, `romance / inner stake`, `flexible branches` を最低限作る。
   - Main Question / Reveal Ladder / Pressure Direction / Heroine Tie は、その arc を運用する索引として作る。
7. 組織や外圧がある場合、`design/organization_cast.md` に主要人物候補を 3-5 人作る。
   - 現場担当、交渉担当、理念担当、内部矛盾担当、ヒロイン生活圏担当の役割を分ける。
8. `current/case.md` に、Concrete Story Arc の現在見える slice として、最初に触れる人・物・場所・記録・期限・if ignored・next visible change を落とす。
...
- `current/case.md` は Heroine-Driven Incident Gate 通過済み Concrete Story Arc の visible/current slice であり、これだけで物語生成が完了したと扱うな。
- 裏側の `design/story_spine.md` が無い時は、visible request だけで初回シーンを始めず、先に最小の Concrete Story Arc を作る。
...
- `handles` は GM 用カードデータであり、NPC の台詞主題ではない。NPC は具体物を自然に指せるが、handle 一覧を読み上げない。
- 初回相談は、物やログより先に「誰が心配していて、何を失いたくないか」を見せる。
- 物、記録、ログ、鍵はヒロインの感情と関係変化へ奉仕させる。単独の謎小物にするな。
- 例文は構造理解用であり、具体物、職業、導入、台詞をコピーしない。Q&A回答、主人公の生活、ヒロインの日常、能力、恋愛の好みから卓固有に再生成する。
- 封筒、鍵、写真、書類、ログ、管理会社などの汎用小物や社会窓口を安易に採用しない。続けて同じ導入型、小物、窓口が出るなら別案を作る。
- NPC に `手順` `範囲` `同意` `守秘` などの運用語を生で言わせるな。職業や文脈上自然な時だけ、作中の言葉へ変換して使う。
- 主人公だけで解けるなら、ヒロインとの shared action / kyun point が生まれる恋愛RPGイベントへ組み直す。
