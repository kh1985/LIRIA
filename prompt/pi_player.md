...
- Q3: 能力名「縁寄せ」。パッシブ。困りごとを抱えた魅力的な人物や、日常の小さな乱れに遭遇しやすくなる。ただし好意は保証されず、厄介事も一緒に来る。
- Q4: スタイルがよく、色気があり、大人っぽい女性が好み。幼すぎるタイプはNG。
- Q5: 初期ヒロインなし。訪問者 / 相談者として1人目が出る。流動型。
- Q6: 最初は、誰かが訪ねてくる形で日常が少し乱れる。
...
作中行動ではなく、GM相談またはメタ命令として扱われる。必要なら `current/gm.md` の `Manga Export Candidates` に2〜3件まで候補を残す。
相談型では実画像生成はプレイヤー確認後にする。
`漫画を出して` `画像も生成して` のような実生成明示型では、追加確認で止めず、裏ジョブとして package 作成から生成投入まで進めてよい。
どちらの場合も、標準の絵柄は漫画/イラストであり、ユーザーが明示しない限り実写/フォトリアルにしない。
