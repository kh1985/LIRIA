...
2. `current/gm.md` `current/player.md` `current/harem.md` `current/case.md` があるか確認し、あればそれを正本として使う。なければ session 直下の `gm.md` `player.md` `harem.md` を fallback として使う
...
7. `current/harem.md` からは、原則として `全ヒロインの現在値 / Heroine Crisis Role / 現在の関係フック / 約束体系 / hidden 深化ベクトル / active なヒロイン間ベクトル / ヒロイン知識スコープ` を引け
8. hotset と current 情報から、今回出るキャラ候補を 2-6 人までに絞る
9. 絞った候補に対応する session 配下の `cast/heroine/*.md` と `cast/npc/*.md` を読む
...
11. `indexes/` がある卓では **`decision_index.md` と `event_index.md` を必ず読む**。特に `decision_index.md` は解決済みフックの確認に必須。これを読まずにフック状態を判断するな。`archive/*` は必要時のみ。再開1ターン目の標準読込に入れるな
12. style/rules.md と使用中の defaults を読み込む
...
これは launcher が勝手に保存する機能ではなく、save file を正本にしたGM側の固定処理だ。`scripts/autosave_turn.sh` は、カウンタ更新、生ログ保存、カウンタリセットを1本化して実ファイルへ確実に反映するための補助コマンドである。
...
- `autosave_turn.sh` は current/*.md の物語状態更新を代行しない。通常の current 更新を済ませたうえで、最後にカウンタ/生ログ保存の補助として実行する
- シーン終了処理で毎回 `bash scripts/autosave_turn.sh <session_name>` を実行し、`current/gm.md` のカウンタを実ファイル上で +1 する
...
- `current/` がある卓では、`current/player.md` `current/gm.md` `current/harem.md` `current/case.md` を正本として扱え
...
1. `current/player.md` `current/gm.md` `current/harem.md` `current/case.md` を更新する
...
13. 会議、告白、報告、共同作戦で知識範囲が動いたら、`current/gm.md` の `知識スコープ台帳` と `current/harem.md` の補助欄を更新する。`Knowledge Boundary / Anti-Leading` と `Anti-Meta Dialogue` の破れも同時に点検する
...
20. 10シーン自動セーブの場合は、`bash scripts/autosave_turn.sh <session_name>` が生ログ保存も実行する。手動セーブ時も、プレイヤーが望むなら `bash scripts/save_rawlog.sh <session_name>` を案内または実行してよい
...
29. プレイヤーに「通常シーンの自動カウントは `bash scripts/autosave_turn.sh <session_name>`、手動の生ログ保存は `bash scripts/save_rawlog.sh <session_name>`、圧縮前チェックは `bash scripts/pre_compress_check.sh <session_name>` が使える」と案内する（初回のみ）
