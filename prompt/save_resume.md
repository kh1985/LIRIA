...
13. 今回出るヒロイン/NPCごとに、`主人公への呼び方 / 相互の呼称関係 / 初対面か再会か / 最後の接触 / 現在のデフォルト外見 / GM-only Body Profile の有無（正確な身長や3サイズを本文数値化しない） / Visual Character Sheet の model sheet status（ヒロインのみ） / tone / 台詞の癖 / Layer 5 or bond snapshot / 知っている固有名詞と suspected 止まりの秘密 / hidden 深化ベクトル snapshot（あれば） / active なヒロイン間ベクトル snapshot（関係するペアだけ）` を1行で内部メモ化しろ
...
16. 成人ヒロインまたは成人主要NPCの `GM-only Body Profile` がある場合は、cast file に保持する。正確な身長や3サイズ等の採寸情報は hotset や本文要約にむやみに出さず、`(gm)` 相談、画像/漫画化、服装差分で必要な時だけ参照する
...
18. `自動セーブ管理` を更新する。手動セーブでは `bash scripts/scene_tick.sh --reset <session_name> "<save_label>"` を実行し、`前回保存からのシーン数` を 0/10 に戻して `最終保存` と `次回自動保存` を書き換える。10シーン自動セーブでは `autosave_turn.sh` がこの reset を担当する
19. 10シーン自動セーブの場合は、`bash scripts/autosave_turn.sh <session_name>` が生ログ保存も実行する。手動セーブ時も、プレイヤーが望むなら `bash scripts/save_rawlog.sh <session_name>` を案内または実行してよい
...
28. プレイヤーに「通常シーンの自動カウントは `bash scripts/autosave_turn.sh <session_name>`、手動の生ログ保存は `bash scripts/save_rawlog.sh <session_name>`、圧縮前チェックは `bash scripts/pre_compress_check.sh <session_name>` が使える」と案内する（初回のみ）
