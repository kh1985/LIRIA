...
頭のてっぺんから爪先まで、髪、顔、体型、首筋、鎖骨、胸元、腰、脚、服装、足元、匂い、仕草を12〜20行で描写し、必要なら `appearance:` に仮置きする。
成人ヒロインの場合は、視線が自然に止まる身体フックを2〜4個入れる。
ただし正確な身長、3サイズ、体重、カップ数のような採寸情報は本文に出さず、必要なら `GM-only Body Profile` に置く。
...
- `height / body type`: 本文/公開資料では数値よりも相対感、骨格、重心、首筋、鎖骨、胸元、腰、ヒップ、脚、服の張り方を含む体の印象。正確な身長は必要なら `GM-only Body Profile` に置く
